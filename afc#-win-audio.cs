...
//     public void OnDefaultDeviceChanged(DataFlow flow, Role role, string defaultDeviceId, int delay)
//     {
//         // Проверяем, что это устройство с ролью Multimedia и типом Render
//         if (flow == DataFlow.Render && role == Role.Multimedia)
//         {
//             GetDeviceInfo(defaultDeviceId, flow, role);
//             StartVolumeMonitoring(delay); // Используем текущий delay вместо фиксированного значения
//         }
//     }
...
//         // Используем delay = 250 по умолчанию, если он не передан через аргументы
//         int delay = 250;
...
//         // Обрабатываем изменения устройства с использованием текущего delay
//         client.OnDefaultDeviceChanged += (flow, role, defaultDeviceId) => client.OnDefaultDeviceChanged(flow, role, defaultDeviceId, delay);
...
class AudioDeviceNotificationClient : IMMNotificationClient
{
    ...
    private float lastVolumeLevel; // Для хранения предыдущего значения громкости
    private System.Timers.Timer? volumeCheckTimer; // Сделаем поле nullable
    public int Delay { get; private set; } // Свойство для хранения задержки
    ...
    public void OnDefaultDeviceChanged(DataFlow flow, Role role, string defaultDeviceId)
    {
        ...
        {
            GetDeviceInfo(defaultDeviceId, flow, role);
            StartVolumeMonitoring(Delay); // Используем текущий delay
        }
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        // Создаем экземпляр клиента уведомлений
        var client = new AudioDeviceNotificationClient(enumerator, delay);
        ...
        // Начинаем мониторинг изменений громкости
        client.StartVolumeMonitoring(delay);
